...
BEGIN
    ...
    BEGIN
        -- Si hi ha error, fer rollback i retornar missatge
        ROLLBACK;
        SET p_nou_id = NULL;
        SET p_error_msg = 'Error al insertar usuari: Excepció SQL';
    END;
...
CREATE PROCEDURE sp_obtenir_usuari_per_email(
    IN p_email VARCHAR(255)
)
BEGIN
    -- Retornar informació completa de l'usuari
    SELECT
        id,
        nom,
        cognoms,
...
CREATE PROCEDURE sp_obtenir_usuari_per_id(
    IN p_usuari_id INT
)
BEGIN
    SELECT
...
CREATE PROCEDURE sp_actualitzar_ultima_connexio(
    IN p_email VARCHAR(255)
)
BEGIN
    UPDATE usuaris
